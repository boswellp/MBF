...
const { QnAMaker } = require('botbuilder-ai'); //MINE aded for qna service
...
this._userKBAccessor = userState.createProperty('kB'); //MINE ADDED
this._userKBAccessor = 'c1'; //MINE ADDED
console.log("change knowledge base = " + this._userKBAccessor.kB) //MINE ADDED
                   ...
                   if (isNaN(str)) 
                         ...
                         if (str.indexOf('Construction Contract',0) != -1)
                               ...
                               if (str != this._userProfileAccessor.profileName) //changing index for c1s to p1s
                                     ...
                                     console.log ("246 SENT INDEX stepContext.context.activity.text = " + stepContext.context.activity.text);
                                      }
                               ...
                               if (categoriesAry.includes(strTemp))
                                    ...
                                    console.log ("268 SENT COLLAPSED stepContext.context.activity.text = " + stepContext.context.activity.text + " with qnaMakerOptions.strictFilters = " + JSON.stringify(qnaMakerOptions.strictFilters) + "to do advanced1 search\n");
                                    }
                                    else
                                    //for stop search
                                    {
...
//First pass - set metadata
       ...
       if (stepContext.context.activity.text == textTemp && textOrig.indexOf('c1si ',0) != -1)
             ...
             console.log("\n563 standard response overruled for advanced search categories - response undefined (WORD NOT IN DOC) ERROR HANDLING NEEDED - STANDARD ERROR SHOWN\n....................................");
             //}


//else // Error handling - word not in doc - goes to end
//{
console.log("\n569 Error handling - word not in doc - executes to end");



        //var responseAnswer = JSON.stringify(response.answers[0].answer) //not use (bug if too many responses?)



//First pass advanced search (get categories for xxxxxx into metatDataAry)

        console.log("\n579 activity.text = " + stepContext.context.activity.text + '; profileNameStored = ' + this._userProfileAccessor.profileName + '; searchTypeStored = ' + this._userSearchAccessor.searchType);

        //var metadataAry = [[],[]];
        var metadataAry = new Array(50).fill(null).map(()=>new Array(5).fill(null));


        //if (this._userProfileAccessor.profileName != undefined && responseAnswer.indexOf('select a category',0) == -1 && this._userSearchAccessor.searchType == "advanced") //change first pass to searchType??
        if (this._userProfileAccessor.profileName != undefined &&  this._userSearchAccessor.searchType == "advanced") //change first pass to searchType??
           {
           if (this._userProfileAccessor.profileName.indexOf('1s',0) != -1)  
               {

               console.log("\n591 in 1st pass answer: Search - select a category ");
...
//Pass 1 - get categories into an array
               ...
               //console.log("\n 639 metadataAry = " + JSON.stringify(metadataAry) + "\n..................................");
               ...
               for (var i = 0; i < 50; i++)
                    ...
                    for (var j = 0; j < 5; j++)
                        ...
                        if (metadataAry[i][j] != null)
                            ...
                            for (var k = 0; k < 50; k++)
                                {
                                //console.log("\n\n657 categoryAry[k] = " + inAry + "; " + i + "; " + j + "; "  + k +"; " + categoryAry[k]);
                                //console.log("658 metadataAry[i][j] value = " + inAry + "; " + i + "; " + j + "; " + JSON.stringify(metadataAry[i][j].value) + "\n")
                            ...
                            if (inAry == false)
                                 {
                                 console.log("\n670 i j metadataAry[i][j]= " + i + "; " + j + "; " + JSON.stringify(metadataAry[i][j]));

                                 if (metadataAry[i][j].name == 'category'){
                                      categoryAry.push(metadataAry[i][j].value);
                                      console.log("674 value .................. = " + metadataAry[i][j].value)
                                      }
...
//Pass 1 create prompts
               ...
               if (categoryAry.length == 0)
                    ...
                    //console.log("\n695 answerPrompt (categoryAry.length=0) = " + JSON.stringify(answerPrompt));
                    }
                    else
                    {
...
//Pass 2 - collapses clauses with advanced search for xxxxx
        ...
        if (profileName != undefined)
           {if (profileName.indexOf('1s',0) != -1 && this._userSearchAccessor.searchType == "advanced1")  
               {
               console.log("\n741 this._userSearchAccessor.searchType = " + this._userSearchAccessor.searchType +"\n......................................\n")
               ...
               for (var i = 0; i < iTotal; i++) 
                  ...
                  if (response.answers[i] != undefined)
                     ...
                     console.log("\n776 i, metadataStr = " + i + " ; " + metadataStr)
                     metadataStr = metadataStr.toString();
                     console.log("\n778 i, medatadataStrtoString = " + i + " ; " + JSON.stringify(metadataStr))
                     ...
                     console.log("\n786 answerPrompt = " + JSON.stringify(answerPrompt))                     
                     console.log("\n787 iTotal-1-i = " + iCount)

                     response.answers[0].context.prompts[iCount] = answerPrompt;

                     console.log("\n791 iTotal-1-i = " + iCount)
